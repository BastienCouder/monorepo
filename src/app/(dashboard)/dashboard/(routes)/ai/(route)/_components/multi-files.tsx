...
import { UploadCloudIcon, FileIcon, Trash2Icon, CheckCircleIcon, AlertCircleIcon, FolderIcon } from 'lucide-react';
...
const MultiFileDropzone: React.FC<InputProps> = ({
    ...
    const uploadFile = (fileState: FileState, userId: string, folderId: string | undefined) => {
        ...
        if (folderId !== '') {
            relativefilePath = `${userId}/${folderId}/${filePath}`;
        } else {
            relativefilePath = `${userId}/${filePath}`;
        }
    ...
    const { getRootProps, getInputProps } = useDropzone({
        onDrop,
        disabled,
        noClick: true, // Empêche le clic pour ouvrir le dialogue de fichier
        noKeyboard: true
    });
