...
import { formatStorage, isNotDefaultFolder } from '@/lib/utils';
...
export interface DirectoryCardProps {
  ...
  color: string;
  Icon: any;
  handleFolderClick: ({ folderName, folderId }: CurrentPath) => void;
    ...
    <li key={key} className="bg-background p-4 rounded-md w-full space-y-4">
      ...
      >
        ...
        {renamingItem === folder.name ? (
          ...
          <h4
            className={`font-bold font-fontRaleway ${isFile(folder.name) ? 'cursor-pointer' : ''}`}
        ...
        {!isFile(folder.name) ? (
          <>
            ...
            <p className="flex text-sm font-bold justify-between">
              ...
              <span>{formatStorage(folder.totalSize)}</span>
            </p>
          </>
        ) : null}
      </div>
