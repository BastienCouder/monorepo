...
import { deleteItems } from '@/server-actions/drive/team/delete-items-drive';
...
export const deleteSelectedRows = (table: Table<Folder | File>, teamId: string, userId: string) => {
  const selectedRows = table.getFilteredSelectedRowModel().rows.map(row => row.original);
  ...
  deleteItems(itemIds, teamId, userId).then(() => {
  }).catch(err => {
    toast.error(catchError(err));
  });
};
